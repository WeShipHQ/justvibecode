import { createPaymentHeaderFromTransaction } from "@/lib/helpers"
...
export async function createSolanaPaymentHeader(
  ...
  // SPL token transfer (including Wrapped SOL)
  if (!paymentRequirements.asset) {
    throw new Error("Missing token mint for SPL transfer")
  }
  const assetAddress = paymentRequirements.asset as string
  ...
  if (!sourceAtaInfo) {
    throw new Error(
      `User does not have an Associated Token Account for ${assetAddress}. Please create one first or ensure you have the required token.`
    )
  }
